...
class BaseExportingCommand(Command):
    ...
    def generate_po(self, language):
        ...
        for filename, filename_ext, filename_no_ext in self.env.xmlfiles:
            lang_po, unmatched = xml2po(filename, language.xml_file(filename_ext))
            write_catalog(language.po_file(filename_no_ext), lang_po)
...
class InitCommand(BaseExportingCommand):
    ...
    def execute(self):
        ...
        for language in languages:
            for filename, filename_ext, filename_no_ext in self.env.xmlfiles:
                if not language.has_xml(filename_ext):
                    dir = path.dirname(language.xml_file(filename_ext))
                    if not path.exists(dir):
                        os.makedirs(dir)
                    f = open(language.xml_file(filename_ext), 'wb')
                    try:
                        f.write("""<?xml version='1.0' encoding='utf-8'?>\n<resources>\n</resources>""")
                    finally:
                        f.close()

                if path.exists(language.po_file(filename_no_ext)):
                    self.i("%s-%s.po exists, skipping." % (filename_no_ext, language.code))
                else:
...
class ExportCommand(BaseExportingCommand):
    ...
    def execute(self):
        ...
        for filename, filename_ext, filename_no_ext in self.env.xmlfiles:
            template_file = "%s.pot" % filename_no_ext
            self.p("Generating %s" % template_file)
            template_pot_file = path.join(env.gettext_dir, template_file)
            default_po = xml2po(filename)
            write_catalog(template_pot_file, default_po)

            if env.options.initial or env.options.overwrite:
                for language in env.languages:
                    if language.has_po(filename_no_ext) and not env.options.overwrite:
                ...
                for language in env.languages:
                    if not language.has_po(filename_no_ext):
                    ...
                    lang_po = read_catalog(language.po_file(filename_no_ext),
                        locale=language.code)
                    lang_po.update(default_po)
                    # TODO: Should we include previous?
                    write_catalog(language.po_file(filename_no_ext),
    ...
    def execute(self):
        for filename, filename_ext, filename_no_ext in self.env.xmlfiles:
            for language in self.env.languages:
                if not path.exists(language.po_file(filename_no_ext)):
                    self.i("Warning: Skipping %s, .po file doesn't exist." % language.code)
                    continue
                po_file = language.po_file(filename_no_ext)
                self.p("Processing %s" % po_file)

                xml_dom = po2xml(read_catalog(po_file))
                write_xml(language.xml_file(filename_ext), xml_dom)
